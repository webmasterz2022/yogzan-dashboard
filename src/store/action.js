  ...
  return async dispatch => {
    try {
      const url = (category && category !== 'Semua') ? 
        `https://yogzan-server-stage.herokuapp.com/gallery/category/${category}?limit=1000${city ? `&city=${city}` : ''}` :
        `https://yogzan-server-stage.herokuapp.com/gallery/?limit=1000${city ? `&city=${city}` : ''}`
      const { data } = await axios({
        method: 'get',
        url
      })
